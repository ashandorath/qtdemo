...
{
  if (this->colourBarPixmap) delete this->colourBarPixmap;
}
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
void Qvis2DTransferFunctionWidget::setBackgroundColourData(int N, int *data)
{
  if (N>0 && data) {
    QImage image( QSize(N,1), QImage::Format_RGB32);
    for (int i=0; i<N; i++) {
      image.setPixel(i, 0, qRgb(data[4*i+0],  data[4*i+1], data[4*i+2]));
    }
    if (this->colourBarPixmap) delete this->colourBarPixmap;
    this->colourBarPixmap = new QPixmap(QPixmap::fromImage(image));
  }
}
//---------------------------------------------------------------------------
...
{
  if (!this->colourBarPixmap) {
    std::cout << "colourBarPixmap is NULL, why? " << std::endl;
    return;
  }
  QImage Image = this->colourBarPixmap->toImage();
  ...
  for (int p=0; p<nregion; p++)
  {
    ...
    double wx = x2-x1;
    double wy = y1-y2;
    double mx = (double)this->colourBarPixmap->width()/this->contentsRect().width();
